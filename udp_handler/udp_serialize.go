...
func (p *Packet) Serialize() ([]byte, error) {
	...
	case RegisterPacket:
		...
		err = binary.Write(buf, binary.BigEndian, []byte(reg.IPv4.To4()))
		if err != nil {
			return nil, err
		}
...
func Deserialize(data []byte) (*Packet, error) {
	...
	case RegisterPacket:
		...
		ipBytes := make([]byte, 4)
		err = binary.Read(buf, binary.BigEndian, ipBytes)
		if err != nil {
			return nil, err
		}
		reg.IPv4 = net.IP(ipBytes)
		packet.Data = reg
...
// Helper function to write a string to a buffer
...
// Helper function to read a string from a buffer
